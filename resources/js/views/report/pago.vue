      ...
      <div class="card text-center">
        <div class="card-header">
          <ul class="nav nav-tabs card-header-tabs">
            ...
            <li class="nav-item">
              <a class="nav-link active" href="#">Pago Manual</a>
            </li>
          </ul>
        </div>
  ...
  methods: {
    ...
    setLink(event, el) {
      if (this.sd && this.ed && this.position) {
        el.href = `${this.$http.options.root}print/pago/${this.sd}/${this.ed}/${this.position}/${this.currentRoom.id}`;
      } else {
        event.preventDefault();
      }
      console.log(`/print/pago/${this.sd}/${this.ed}/${this.position}`);
    },
    find() {
      this.$http.get(`operations/byDateRangeAndPosition/${this.sd}/${this.ed}/${this.position}/2/${this.currentRoom.id}`).then(response => {
        console.log(response);
